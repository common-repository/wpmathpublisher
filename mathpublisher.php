...
//$dirfonts=$_SERVER["DOCUMENT_ROOT"]."/wp-content/plugins/wpmathpublisher/fonts";
$dirfonts=dirname(__FILE__).DIRECTORY_SEPARATOR."fonts";

// absolute path to the images directory
//$dirimg=$_SERVER["DOCUMENT_ROOT"]."/wp-content/plugins/wpmathpublisher/img";
$dirimg=dirname(__FILE__).DIRECTORY_SEPARATOR."img";
...
switch($texte)
	...
	$backCol = wpMathPublisher_getBackgroundColor($img);
	ImageFilledRectangle($img,0,0,1,$haut,$backCol);
	break;
	case '~':
	$img = ImageCreate(1, max($haut,1));
	//$blanc=ImageColorAllocate($img,255,255,255);
	...
	$fontColor = wpMathPublisher_getFontColor($img);
	ImageFilledRectangle($img,0,0,6,$haut,$backCol);
	ImageFilledRectangle($img,2,0,2,$haut,$fontColor);
	ImageFilledRectangle($img,4,0,4,$haut,$fontColor);
	break;
	case '|':
	...
	//$noir=ImageColorAllocate($result,0,0,0);
	$fontColor = wpMathPublisher_getFontColor($img);
	ImageFilledRectangle($img,0,0,5,$haut,$backCol);
	ImageFilledRectangle($img,2,0,2,$haut,$fontColor);
	break;
	case 'right':
	...
	$tmp_fontColor = wpMathPublisher_getFontColor($tmp_img);
	//$tmp_blanc=ImageColorAllocate($tmp_img,255,255,255);
	//$tmp_blanc=imagecolortransparent($tmp_img,$tmp_blanc);
	$tmp_backCol = wpMathPublisher_getBackgroundColor($tmp_img);
	ImageFilledRectangle($tmp_img,0,0,$tmp_largeur,$tmp_hauteur,$tmp_backCol);
	ImageTTFText($tmp_img, $t, 0,0,$tmp_hauteur,$tmp_fontColor, $font,$texte);
	$toutblanc=true;
	$sx = $sy = $ex = $ey = -1;
	...
	//$blanc=imagecolortransparent($img,$blanc);
	$backCol = wpMathPublisher_getBackgroundColor($img);
	ImageFilledRectangle($img,0,0,$nx+4,$ny+4,$backCol);
	...
	$tmp_backCol = wpMathPublisher_getBackgroundColor($tmp_img);
	ImageFilledRectangle($tmp_img,0,0,$tmp_largeur,$tmp_hauteur,$tmp_backCol);
	ImageTTFText($tmp_img, $t,0,5,$tmp_hauteur/2,$tmp_fontColor,$font,$texte);
	$img=$tmp_img;
	$toutblanc=true;
	$sx = $sy = $ex = $ey = -1;
	for ($y = 0; $y < $tmp_hauteur; $y++)
	...
	$tmp_backCol = wpMathPublisher_getBackgroundColor($tmp_img);
	ImageFilledRectangle($tmp_img,0,0,$tmp_largeur,$tmp_hauteur,$tmp_backCol);
	ImageTTFText($tmp_img, $t, 0,0,$tmp_hauteur/4,$tmp_fontColor, $font,$texte);
...
//$blanc=imagecolortransparent($img,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($img);
ImageFilledRectangle($img,0,0,$dx,$dy,$backCol);
//ImageRectangle($img,0,0,$dx-1,$dy-1,$noir);
ImageTTFText($img, $taille, $angle, 0, -min($hdim[5], $hdim[7]), $fontColor, $font, $texte);
return $img;
}
...
ImageFilledRectangle($img,0,0,$dx,$dy,$backCol);
//ImageRectangle($img,0,0,$dx-1,$dy-1,$noir);
ImageTTFText($img, $taille, 0, 0, -min($hdim[5], $hdim[7]), $fontColor, $font, $texte);
return $img;
}
...
//$noir=ImageColorAllocate($result,0,0,0);
$fontColor = wpMathPublisher_getFontColor($result);
//$blanc=ImageColorAllocate($result,255,255,255);
//$blanc=imagecolortransparent($result,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($result);
ImageFilledRectangle($result,0,0,$largeur-1,$hauteur-1,$backCol);
...
ImageFilledRectangle($result,0,0,$largeur+4,$hauteur-1,$backCol);
...
imageline($result, $largeurrac-2,2, $largeurrac+$largeurexp+2,2, $fontColor);
...
$fontColor = wpMathPublisher_getFontColor($imggauche);
//$blanc=ImageColorAllocate($imggauche,255,255,255);
//$blanc=imagecolortransparent($imggauche,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($imggauche);
ImageFilledRectangle($imggauche,0,0,$largeurgauche-1,$hauteurgauche-1,$backCol);
...
$fontColor = wpMathPublisher_getFontColor($imgfin);
//$blanc=ImageColorAllocate($imgfin,255,255,255);
//$blanc=imagecolortransparent($imgfin,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($imgfin);
ImageFilledRectangle($imgfin,0,0,$largeur-1,$hauteur-1,$backCol);
...
//$noir=ImageColorAllocate($imgfin,0,0,0);
$fontColor = wpMathPublisher_getFontColor($imgfin);
//$blanc=ImageColorAllocate($imgfin255,255,255);
//$blanc=imagecolortransparent($imgfin,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($imgfin);
ImageFilledRectangle($imgfin,0,0,$largeur-1,$hauteur-1,$backCol);
...
//$blanc=imagecolortransparent($imgfin,$blanc);
$backCol = wpMathPublisher_getBackgroundColor($imgfin);
ImageFilledRectangle($imgfin,0,0,$largeurfin-1,$hauteurfin-1,$backCol);
$i=0;
$h=$padding/2-1;
for($ligne = 0; $ligne <$nbligne; $ligne++)
	{
...
$i=0;
$h=$padding/2-1;
if (substr($typeligne,0,1)=="1") ImageLine($imgfin,0,0,$largeurfin-1,0,$fontColor);
for($ligne = 0; $ligne <$nbligne; $ligne++)
	{
	$l=$padding/2-1;
	if (substr($typecolonne,0,1)=="1") ImageLine($imgfin,0,$h-$padding/2,0,$h+$hauteur_ligne[$ligne]+$padding/2,$fontColor);
	for($col = 0; $col <$nbcolonne; $col++)
		{
		if ($i< count($this->noeuds[3]->noeuds))
			{
			ImageCopy($imgfin,$img[$i],$l+ceil($largeur_colonne[$col]-$largeur[$i])/2,$h+$dessus_ligne[$ligne]-$base[$i], 0, 0,$largeur[$i],$hauteur[$i]);
			if (substr($typecolonne,$col+1,1)=="1") ImageLine($imgfin,$l+$largeur_colonne[$col]+$padding/2,$h-$padding/2,$l+$largeur_colonne[$col]+$padding/2,$h+$hauteur_ligne[$ligne]+$padding/2,$fontColor);
			}
		$l+=$largeur_colonne[$col]+$padding;
		$i++;
		}
	if (substr($typeligne,$ligne+1,1)=="1") ImageLine($imgfin,0,$h+$hauteur_ligne[$ligne]+$padding/2,$largeurfin-1,$h+$hauteur_ligne[$ligne]+$padding/2,$fontColor);
	$h+=$hauteur_ligne[$ligne]+$padding;
 	}
$this->image=$imgfin;
$this->base_verticale=imagesy($imgfin)/2;
...
if ($v==0)
	...
	// ImagePNG($formula->image,$dirimg."/math_".$v."_".$nameimg);  // original.. extended to add some border to the image /space between image-border and content
	
	// Added by Timm Severin
	$imageWidth = imagesx($formula->image);
	$imageHeight = imagesy($formula->image);
	$newImg = ImageCreate($imageWidth + 4, $imageHeight + 8);
	imagefill($newImg, 1, 1, wpMathPublisher_getBackgroundColor($newImg));
	imagecopy($newImg, $formula->image, 2, 4, 0, 0, $imageWidth, $imageHeight);
	ImagePNG($newImg,$dirimg."/math_".$v."_".$nameimg);
...
foreach ($regs as $math) 
	...
	//Bearbeitung von $t, so dass HTML-Code-teile (&amp; rausfallen)
	$t = str_replace("&amp;", "&", $t);
